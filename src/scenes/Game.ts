...
export class Game extends Scene {
    ...
    private setupPayoutBoxes() {
        ...
        for (const [key, value] of Object.entries(payouts)) {
            ...
            this.add.text(0, y, `${key.toUpperCase()}`, style2).setOrigin(0, 0).setPosition(box.inner.x - box.inner.width / 2 + 5, box.inner.y - box.inner.height / 2 + (y % 6) * 30);
    ...
    private setupCreditsAndBet() {
        ...
        const betValue = this.add.text(0, 0, '1', { fontFamily: 'upheaval', fontSize: '24pt', color: '#ffffff', shadow: { color: '#333333', offsetX: 2, offsetY: 2, blur: 0, fill: true, stroke: false } }).setOrigin(1, 1);
